...
import com.weolopez.butler.ButlerInstance.Status;
...
public class ButlerTrait extends Trait implements Toggleable {

    private Butler plugin = null;

    private boolean isToggled = true;

    public ButlerTrait() {
        super("butler");
        plugin = (Butler) Bukkit.getServer().getPluginManager().getPlugin("Butler");
    }

    private ButlerInstance thisInstance;
    ...
    public void load(DataKey key) throws NPCLoadException {
        ...
        thisInstance.butlerHealth = key.getDouble("Health", plugin.getConfig().getInt("DefaultStats.Health", 20));
        thisInstance.butlerRange = key.getInt("Range", plugin.getConfig().getInt("DefaultStats.Range", 10));
        thisInstance.RespawnDelaySeconds = key.getInt("RespawnDelay", plugin.getConfig().getInt("DefaultStats.Respawn", 10));
        thisInstance.butlerSpeed = (float) (key.getDouble("Speed", plugin.getConfig().getDouble("DefaultStats.Speed", 1.0)));
        thisInstance.butlerWeight = key.getDouble("Weight", plugin.getConfig().getDouble("DefaultStats.Weight", 1.0));
    ...
    public ButlerInstance getInstance() {
        return thisInstance;
    }
    ...
    private void ensureInst() {
        if (thisInstance == null) {
            thisInstance = new ButlerInstance(plugin);
    ...
    public void onRemove() {
        ...
        //	plugin = (Butler) Bukkit.getPluginManager().getPlugin("Butler");
    ...
    public void onDespawn() {
        ...
        if (thisInstance != null) {
            ...
            thisInstance.butlerStatus = Status.isDEAD;
    ...
    public void save(DataKey key) {
        ...
        key.setDouble("Health", thisInstance.butlerHealth);
        key.setInt("Range", thisInstance.butlerRange);
        key.setInt("RespawnDelay", thisInstance.RespawnDelaySeconds);
        key.setDouble("Speed", (double) thisInstance.butlerSpeed);
        key.setDouble("Weight", thisInstance.butlerWeight);
